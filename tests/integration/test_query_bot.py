...
class TestQueryByStationCurrentDate:
    ...
    def test_query_by_station_current_date_max_date(self, prepare_test_db):
        q = query_by_station_current_date(prepare_test_db, 'okko', 'address1')
    ...
    def test_query_by_station_current_date_with_date(self, prepare_test_db):
        cur_date = date.today() - timedelta(days=1)
        q = query_by_station_current_date(prepare_test_db, 'okko', 'address1', date.today() - timedelta(days=1))
        assert q.count() == 4
        assert (abs(q[0][0] - Decimal(28.2))) < 0.01
        assert q[0][1] == datetime(cur_date.year, cur_date.month, cur_date.day, 22, 39)
        assert q[0][2] == '92'

    def test_query_by_station_current_date_no_info(self, prepare_test_db):
        q = query_by_station_current_date(prepare_test_db, 'okko', 'address1', date.today() + timedelta(days=2))
        assert q.count() == 0
...
class TestQueryByStationMinPrice:
    def test_query_by_station_min_price_max_date(self, prepare_test_db):
        q = query_by_station_min_price(prepare_test_db, '95')
        max_date = datetime(date.today().year, date.today().month, date.today().day, 10, 40)
        assert q[0] == (30, max_date, '95', 'address1', 'okko')

    def test_query_by_station_min_price_with_date(self, prepare_test_db):
        cur_date = date.today() - timedelta(days=1)
        q = query_by_station_min_price(prepare_test_db, '95', cur_date)
    ...
    def test_query_by_station_min_price_no_info(self, prepare_test_db):
        q = query_by_station_min_price(prepare_test_db, '95', date.today() + timedelta(days=2))
        assert q.count() == 0


class TestQueryAVGPricePeriod:
    def test_query_avg_price_period_without_date(self, prepare_test_db):
        q = query_avg_price_period(prepare_test_db, '92')
        assert q.count() == 6
    ...
    def test_query_avg_price_period_with_date(self, prepare_test_db):
        cur_date = date.today() - timedelta(days=1)
        q = query_avg_price_period(prepare_test_db, '92', cur_date, date.today())
        assert q.count() == 4
        assert (abs(q[0][0] - Decimal(28.25))) < 0.01
        assert q[0][1] == date.today()
        assert q[0][2] == '92'
    ...
    def test_query_avg_price_period_no_info(self, prepare_test_db):
        q = query_avg_price_period(prepare_test_db, '92', date.today() + timedelta(days=2),
                                   date.today() + timedelta(days=3))
        assert q.count() == 0


class TestQueryAllPricePeriod:
    def test_query_all_price_period_without_date(self, prepare_test_db):
        q = query_all_price_period(prepare_test_db)
        assert q.count() == 40
        max_date = datetime(date.today().year, date.today().month, date.today().day, 10, 40)
        assert q[0] == (28, max_date, '92', 'address1', 'okko')

    def test_query_all_price_period_with_date(self, prepare_test_db):
        cur_date = date.today() - timedelta(days=1)
        q = query_all_price_period(prepare_test_db, cur_date, date.today())
        assert q.count() == 24
        assert q[0] == (
        28, datetime(date.today().year, date.today().month, date.today().day, 10, 40), '92', 'address1', 'okko')

    def test_query_avg_price_period_no_info(self, prepare_test_db):
        q = query_all_price_period(prepare_test_db, date.today() + timedelta(days=2), date.today() + timedelta(days=3))
        assert q.count() == 0
